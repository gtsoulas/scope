...
The main idea that we pursued in this paper is that a careful look at a typologically rich set of \textit{wh-}questions and their derivation focusing specifically on the mechanisms determining scope of \textit{wh-}elements provides a particularly good lens through which scope assignment in general can be fruitfully considered.  We concluded that the most promising way to look at \textit{wh-}scope is through \textit{selective spellout} which we suggested is a better way to understand the distributions than its relative, LF-movement, whose basic mechanisms seem to run counter to current theoretical understanding of syntactic derivations.
